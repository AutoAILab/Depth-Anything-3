...
class EventHandlers:
    ...
    def gradio_demo(
        ...
        gr.update,  # gs download visibility update
        Optional[str],  # gs download value (ply path)
        ...
        if not os.path.isdir(target_dir) or target_dir == "None":
            return (
                ...
                None,
                None,
                gr.update(visible=False),  # gs_download
                None,  # gs_download value
        ...
        gs_info_visible = True
            ...
            try:
                ...
                gsply_path = sorted(glob(os.path.join(target_dir, "gs_ply", "*.ply")))[-1]
                gs_ui_visible = True
                gs_info_visible = False
            except IndexError:
                gsvideo_path = None
                gsply_path = None
                print("3DGS video or ply not found, but infer_gs was enabled")
        ...
        return (
            ...
            gsvideo_path,
            gr.update(visible=gs_ui_visible),  # gs_download visibility
            gsply_path,  # gs_download value
    ...
    def load_example_scene(self, scene_name: str, examples_dir: str = None) -> Tuple[
        ...
        if target_dir and target_dir != "None":
            ...
            # Check for cached 3DGS ply
            gs_ply_dir = os.path.join(target_dir, "gs_ply")
            if os.path.exists(gs_ply_dir):
                try:
                    gs_plies = sorted(glob(os.path.join(gs_ply_dir, "*.ply")))
                    if gs_plies:
                        gs_ply_path = gs_plies[-1]
                        gs_ui_visible = True
                        gs_info_visible = False
        ...
        return (
            ...
            gs_video_path,
            gr.update(visible=gs_ui_visible),
            gs_ply_path,
